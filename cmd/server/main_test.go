...
// mustRouter builds the router and fails the test on a construction error.
func mustRouter(t *testing.T, options routerOptions) http.Handler {
	t.Helper()
...
func TestRouterProtectedPhase3RoutesRequireAuthentication(t *testing.T) {
	...
	router, err := newRouter(routerOptions{logger: testLogger(), cfg: cfg, crypto: testCrypto(), k8s: testK8s(), oidcProvider: routerFakeProvider{}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRouterReadyzReturns503WithoutConfig(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	mustRouter(t, routerOptions{logger: testLogger(), cfg: cfg, crypto: testCrypto(), k8s: testK8s()}).ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/readyz", nil))
...
func TestRouterReadyzReturns200WithConfig(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	mustRouter(t, routerOptions{logger: testLogger(), cfg: testConfig(), crypto: testCrypto(), k8s: testK8s()}).ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/readyz", nil))
...
func TestRouterDoesNotExposeProtectedRoutes(t *testing.T) {
	for _, path := range []string{
		...
		rr := httptest.NewRecorder()
		mustRouter(t, routerOptions{logger: testLogger(), cfg: testConfig(), crypto: testCrypto(), k8s: testK8s()}).ServeHTTP(rr, httptest.NewRequest(http.MethodGet, path, nil))
